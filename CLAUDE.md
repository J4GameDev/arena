...
- **At the source** — not biological at all. Not in v0.1, and no player ever sees it — but it is designed, so that what leaks out of it stays consistent.

**The whole road is designed on paper; only band one is built.** The owner's call on 4 Sep 2026 when the design doc was scoped: every band, its creatures and the thing at the source get written down, so band one is drawn knowing what it is the shallow end of. This moves the v0.1 build scope by nothing — writing a band is not building one, and the Out list stands unchanged. **The hard rule survives it: the thing at the source never speaks, never notices us, and is never revealed.** Designing it is how we keep what it leaks consistent, not permission to show it.
...
**The Berserkers carry a double-bladed greataxe** since the end of 3 Sep 2026: the axe icon candidates came back two-bladed, and the owner chose to redraw the class to match the icon rather than fight the model. Both were rebuilt from the base bodies with the chosen icon as a weapon reference.

**Every idle holds its weapon in both hands or in hand — nothing rests on a shoulder.** Learned the evening of 4 Sep 2026, and it is an animation rule wearing a drawing rule's clothes: an animator keeps whatever grip frame 0 has and will never invent the second hand. The greataxe carried across the shoulder failed to give a clean swing in both tools, every run. A sheet of the same Berserker in a two-handed ready stance — axe low in front, both hands on the handle — came back sixteen for sixteen with the grip right, and Retro Diffusion's attack preset turned the owner's pick into the first clean two-handed chop of the day. The two installed greataxe sprites still carry the axe on the shoulder and are wrong under this rule; the redraw below is what corrects them.
...
**Practical.** Art is pixel art via **Retro Diffusion**, chosen the evening of 4 Sep 2026. Stills are drawn locally: their Aseprite extension runs their pixel model on the owner's RTX 3060 at about five seconds an image, bought once for $65 and free per image after that, with the owner at the keyboard and the prompt copied out of `design/`. Their online connector draws the same stills at 18 cents each when a batch has to run without him, and their attack preset animates a swing for 14 cents. Nothing local animates. Cost estimates are free — check before spending. The style is `rd_pro__default`; prompts describe the subject and never say "pixel art"; a standalone asset is asked for "on a plain white background" and has the background removed. The key lives in the owner's user settings and never in the repo.

Keep the same palette family, lighting direction, and sprite dimensions within a category. Direction drifts fast when assets are generated one at a time, so look at what already exists before adding to it.

**PixelLab is left behind for this project.** Its lessons were never about PixelLab and still hold. A shape the model does not know from words needs a reference picture: "double-bladed greataxe" drew single blades until an axe icon was passed as a weapon reference, after which every candidate had two. The style image leaks its subject: a hood as style reference turned a sword run into sixty-four hoods, so items take an item of another kind as their style reference and characters take a character. A run that derails into random townsfolk and potions is re-run with a seed before anything else is changed. And its animator could not be trusted with hands — told every way to keep both on a greataxe's handle, it re-gripped, grabbed the blade or passed the axe across in nine runs, and pinning drawn stills as first and last frames fixed the order of a motion but never the grip. The stance rule above is what came of that.

**ComfyUI was installed and deleted the same night**, 4 Sep 2026. SDXL with the Pixel-Art-XL add-on, run on the owner's own card so it would cost nothing an image, produced turnaround sheets and camera-facing figures he called abysmal. Local generation on this project means the Aseprite extension, not a stack. Do not propose it again.

**Everything drawn so far is being redrawn in Retro Diffusion's hand.** The owner's call on 4 Sep 2026, taken with the pipeline: one tool's style across the whole game rather than two seams, which means all twenty-one sprites, both scenes, every icon and Oswald's portrait — not only what comes next. This is what makes the design doc load-bearing: it is the specification the redraw is drawn from, so a character described loosely there comes back loose. **The second body is drawn from the first, never from words:** "the same, but her" worked for the Warden and derailed twice for the Berserker, and the recipe that held was the base woman plus the same weapon reference. The town's three hover lines are traced from `town.png` by `scripts/cutouts.py` and have to be regenerated when that scene is.
...
- **The design is written before it is drawn.** The owner's call at the end of 4 Sep 2026, after a day in which generations kept deciding creative questions by accident. Character and world choices live in `design/`, written first and specific enough that a prompt is a line copied out of the doc. A generation may reveal that a choice was wrong; it never gets to make one. When the art and the doc disagree, the doc is what was decided — unless the owner moves the fiction to meet the art on purpose, as he did when the strayed sprites came back looking like ordinary men and became bandits.
...
design/    The written design: the characters and the world, band by band. Prompts are lines copied out of here.
...
content/   Screenshots, art experiments, banners and post material. Git-ignored in full: none of it ships.
...
**Built and tested — 91 tests:** the combat simulation (attack-speed timeline, five meters — Rage, Focus, Resolve, Snare, Mana — crit on both sides, block, evasion, lifesteal, initiative, resource retention, the snare, several monsters at once), hunts (encounter tables, ambushes, carried health, rations, the fall tax), five archetypes all offered at the rack, each beside the chosen body's figure carrying it, eight monsters (a teacher, four band-one animals, two bandits, a gate), eight equipment slots (two rings), 19 affix kinds, weighted and tilted rolls, crafting, the cookfire, the Hunter's Pack, run state and saving (save version 7, with the first migration: a version 6 save keeps its ring), the opening, the town screen with its hover lines, the gear table, the fight view with several foes, twenty-one sprites (ten heroes on two bodies, the two base bodies, Oswald twice, two bandits, four animals, the gate), seven slot icons (five armor, two trinket), nine engraved glyphs, six weapon icons, Oswald's portrait, and two scenes. Four harnesses: `npm run fight`, `npm run balance`, `npm run hunts`, `npm run outliers`. And one content tool: `npm run shot` drives a headless Chromium against the live build, hunts until the fight you name comes up, freezes it a few blows in and saves the scene — every build is a shareable picture as well as a link.
...
3. **The redraw, in Retro Diffusion's hand.** Twenty-one sprites, two scenes, the icons and Oswald's portrait, each from a prompt copied out of `design/`. Every idle in the two-handed ready stance, nothing on a shoulder. Locally in Aseprite this costs nothing an image and the owner clicks; through the connector it is 18 cents a still, roughly $8 for the set. The town's hover lines are re-traced by `scripts/cutouts.py` when `town.png` is redrawn. Art still comes as options, never a one-shot install.
4. **Drawn attack animations**, once the idles are redrawn in the ready stance — that stance is what made the swing work at all. Retro Diffusion's attack preset, 14 cents a swing. The CSS whole-sprite swing from 4 Sep 2026 is still uncommitted in the working tree (`src/view/fight-view.ts`, `src/style.css`) — a wind-up and lunge that moves the whole sprite, a knock-back and a pale flash on a struck foe, a hop on a miss, the damage number and log line delayed to the instant the blow lands, four strengths of it on a private review page. It is the fallback if drawn swings stall, and it is not thrown away until they land.
5. **Oswald's and the gate's words in town.** The panels open; the lines in them are placeholders. Scenes in the owner's voice, not briefings. The owner writes; a critic may check a draft against the copy rules here, and nothing more.
6. **Band-one animals that actually read as strange.** At 64px the one-detail wrongness did not render; what came back was normal animals with red eyes. This is now the redraw's problem to solve rather than a separate pass: lead the prompt with a wrongness big enough to survive 64px, or use a larger canvas, and judge each sheet with a blind test — a fresh reader asked what is wrong with the animal.
7. **The smithy**, held until there is something for it to do. The doc names the first honest job it could take: the place deep-band materials go when the tanner cannot work them.
